...
func rewriteValueAMD64_OpAMD64ANDLmodify(v *Value) bool {
	...
	// result: (BTRLmodify [off] {sym} ptr (ANDLconst [31] <t> x) mem)
	for {
		off := v.AuxInt
		sym := v.Aux
		ptr := v_0
		...
		x := s.Args[1]
		s_0 := s.Args[0]
		if s_0.Op != OpAMD64MOVLconst || s_0.AuxInt != 1 {
			break
		}
		mem := v_2
...
func rewriteValueAMD64_OpAMD64ANDQmodify(v *Value) bool {
	...
	// result: (BTRQmodify [off] {sym} ptr (ANDQconst [63] <t> x) mem)
	for {
		off := v.AuxInt
		sym := v.Aux
		ptr := v_0
		...
		x := s.Args[1]
		s_0 := s.Args[0]
		if s_0.Op != OpAMD64MOVQconst || s_0.AuxInt != 1 {
			break
		}
		mem := v_2
...
func rewriteValueAMD64_OpAMD64MOVLstore(v *Value) bool {
	...
	// cond: y.Uses==1 && l.Uses==1 && clobber(y, l)
	// result: (BTCLmodify [off] {sym} ptr (ANDLconst [31] <t> x) mem)
	for {
		off := v.AuxInt
		sym := v.Aux
		ptr := v_0
	...
	// cond: y.Uses==1 && l.Uses==1 && clobber(y, l)
	// result: (BTSLmodify [off] {sym} ptr (ANDLconst [31] <t> x) mem)
	for {
		off := v.AuxInt
		sym := v.Aux
		ptr := v_0
...
func rewriteValueAMD64_OpAMD64MOVQstore(v *Value) bool {
	...
	// cond: y.Uses==1 && l.Uses==1 && clobber(y, l)
	// result: (BTCQmodify [off] {sym} ptr (ANDQconst [63] <t> x) mem)
	for {
		off := v.AuxInt
		sym := v.Aux
		ptr := v_0
	...
	// cond: y.Uses==1 && l.Uses==1 && clobber(y, l)
	// result: (BTSQmodify [off] {sym} ptr (ANDQconst [63] <t> x) mem)
	for {
		off := v.AuxInt
		sym := v.Aux
		ptr := v_0
